...
HRESULT StoryScene::init(void)
{
	...
	string path = "Resources/Sounds/storyScript/";
...
void StoryScene::update(void)
{
	...
	if (KEYMANAGER->isOnceKeyDown(VK_SPACE) && _textIndex != 9 && _textIndex < TEXTNum-1)
	{
		...
		else if (_textBufferCnt == wcslen(_text[_textIndex].script))
		{
			...
			if (_bgIndex < BgImageNUM && _textIndex == _bgImage[_bgIndex]._textIndex)
			{
				if(_bgIndex != 4) _bgIndex++;
			}
